...
public interface VerificationTokenRepository extends JpaRepository<VerificationToken, UUID> {
  ...
  @Query("SELECT vt FROM VerificationToken vt WHERE vt.user.id = :userId AND vt.type = :type AND vt.consumedAt IS NULL AND vt.expiresAt > :now")
  Optional<VerificationToken> findActiveTokenByUserAndType(@Param("userId") UUID userId,
